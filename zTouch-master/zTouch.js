function Swipe(tBox, args) {
    ...
    (function () {
        ...
        tPoint.bL = _offset.left;//相对文档left的偏移量
        tPoint.bT = _offset.top;//相对文档top的偏移量
...
//如果加载了jquery或者zepto 就调用这个匿名函数，当然，会把jquery或者zepto传进去
if (window.jQuery || window.Zepto) {
    //这是一个匿名函数自执行
    (function (a) {
        //如果你加载了zepto就可以把这个移出去，没必要判断
        a.fn.Swipe = function (b) {
            //zepto的链式调用
            return this.each(function () {
                //给一个div添加一个自定义属性Swipe,同时调用Swipe()
